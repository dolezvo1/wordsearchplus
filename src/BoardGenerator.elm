    ...
    let
        ...
        generateWordSearchAux ( entropy, ( typeSamples, allWordsRandom ) ) =
            ...
            createWordSearch words allDirections maxDensity entropy
                |> Result.map
                    (\( grid, ws, _ ) ->
                        ( ws |> List.map (\( _, ( wt, ow, d ), p ) -> ( wt, ( ow, d ), p ))
...
createWordSearch words allDirections maxDensity entropy =
    let
        newEmptyGrid =
            getEmptyWordSearchGrid (getMinimumGridSize (words |> List.map Tuple.first) maxDensity)

        uWords =
            words |> List.map (\( w, data ) -> ( String.toUpper w, data ))
    in
    if (maxDensity <= 0) || (maxDensity > 1) then
        Err "Invalid max density."

    else if words == [] then
        Ok ( [], [], entropy )

    else
        createWordSearchAux allDirections uWords [] newEmptyGrid uWords entropy
...
createWordSearchAux allDirections words placedWordsAcc grid allWords entropy =
    case words of
        [] ->
            let
                uniqueCharacters =
                    allWords |> List.concatMap (Tuple.first >> String.toList)
            in
            fillBlanks uniqueCharacters grid entropy |> Result.map (\( g, e ) -> ( g, placedWordsAcc, e ))

        ( w, data ) :: restOfWords ->
            if w == "" then
                Err "Invalid input word: Words length must be >= 1."

            else
                -- cant insert empty word into a grid
                case getRandomPlacement allDirections w grid 0 entropy of
                    Err e ->
                        Err e

                    Ok ( ( ( sx, sy ) as sp, dir ), entropy2 ) ->
                        if ( sp, dir ) == ( ( 0, 0 ), Up ) then
                            -- word cant be placed - bigger grid needed
                            let
                                newBiggerDimension =
                                    floor ((List.length grid |> toFloat) * 1.5)

                                newBiggerGrid =
                                    getEmptyWordSearchGrid newBiggerDimension
                            in
                            createWordSearchAux allDirections allWords [] newBiggerGrid allWords entropy2

                        else
                            let
                                updatedGrid =
                                    addWord w ( sp, dir ) grid
                                ...
                                newWord =
                                    ( w, data, ( sp, ( sx + dx * remainingLen, sy + dy * remainingLen ) ) )
                            in
                            createWordSearchAux allDirections restOfWords (newWord :: placedWordsAcc) updatedGrid allWords entropy2
...
getMinimumGridSize words maxDensity =
    let
        ...
        gridSizeByWordLength =
            getMinimumGridSizeByWordLength words
    in
    max gridSizeByDensity gridSizeByWordLength
...
getMinimumGridSizeByDensity words maxDensity =
    let
        totalCharacters =
            words |> List.map String.length |> List.sum

        minimumCellsNeeded =
            ceiling (toFloat totalCharacters / maxDensity)
    in
    nextSquareNumber minimumCellsNeeded 0
...
nextSquareNumber number currentGuess =
    if (currentGuess * currentGuess) > number then
        currentGuess

    else
        nextSquareNumber number (currentGuess + 1)
...
getMinimumGridSizeByWordLength words =
    getMinimumGridSizeByWordLengthAux words 0
...
getMinimumGridSizeByWordLengthAux words currentMaxWordLength =
    case words of
        ...
        w :: remainingWords ->
            getMinimumGridSizeByWordLengthAux remainingWords (max (String.length w) currentMaxWordLength)
...
getEmptyWordSearchGrid n =
    '~' |> List.repeat n |> List.repeat n
...
getRandomPlacement allDirections word grid attempts entropy =
    if attempts >= List.length grid * List.length grid * 4 then
        Ok ( ( ( 0, 0 ), Up ), entropy )

    else
        case getRandomOrientation allDirections entropy of
            Err e ->
                Err e
                ...
                case getRandomPosition (List.length grid) entropy2 of
                    ...
                    Ok ( position, entropy3 ) ->
                        let
                            ...
                            correct =
                                isCorrectPlacement word placement grid
                        in
                        if correct then
...
getRandomPosition dimension entropy =
    case entropy of
        c :: r :: remainingEntropy ->
            Ok ( ( modBy dimension c, modBy dimension r ), remainingEntropy )
...
getRandomOrientation allDirections entropy =
    let
        orientations =
            [ Forward, Down, DownForward, Back, Up, UpForward, UpBack, DownBack ]
    in
    case entropy of
        [] ->
            Err "Insufficient entropy"
...
isCorrectPlacementAux word (( cc, cr ) as currentPosition) (( dc, dr ) as direction) grid =
    case String.uncons word of
        ...
        Just ( l, restOfWord ) ->
            if isOutOfBoundsC2 currentPosition grid then
                False

            else
                case grid |> List.Extra.getAt cr |> Maybe.andThen (List.Extra.getAt cc) of
                    Nothing ->
                        False
                    ...
                    Just ol ->
                        if (ol /= '~') && (ol /= l) then
                            False
                            -- this cell isnt free, and isnt a  match

                        else
                            isCorrectPlacementAux restOfWord ( cc + dc, cr + dr ) direction grid



-- this cell is free or is a  match
{- outOfBoundsC2
...
isOutOfBoundsC2 (( c, r ) as position) grid =
    (c >= List.length grid) || (r >= List.length grid) || (c < 0) || (r < 0)
...
addWordAux word (( cc, cr ) as currentPosition) (( dc, dr ) as direction) grid =
    case String.uncons word of
        ...
        Just ( l, word2 ) ->
            addWordAux word2 ( cc + dc, cr + dr ) direction (addLetter l grid currentPosition)
...
addLetter l grid (( c, r ) as position) =
    case grid of
        ...
        row :: restOfGrid ->
            if r == 0 then
                addLetterAux l row c :: restOfGrid

            else
                row :: addLetter l restOfGrid ( c, r - 1 )
        ...
        l :: restOfRow ->
            case fillBlanksAux characters restOfRow entropy of
                ...
                Ok ( remainder, entropy2 ) ->
                    case
                        if l == '~' then
                            getRandomChar characters entropy2
...
getRandomChar characters entropy =
    case entropy of
        ...
        e :: remainingEntropy ->
            Ok ( characters |> List.Extra.getAt (modBy (List.length characters) e) |> Maybe.withDefault '~', remainingEntropy )
